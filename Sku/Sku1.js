...
function SC_djs() {
    ...
    for (var i = 0; i < Object.keys(sy_djs).length; i++) {
        ...
        divs.addEventListener('contextmenu', function(e) {
            ...
            //显示设置
            sy_shezhi_gn.style.display = 'block';
            sy_shezhi_gn_cmm.style.display = 'block';
...
sy_shezhi_gn_cmm.addEventListener('click', function(e) {
    ...
    if (sy_gn_zhixian == '倒计时') {
        ...
        var sy_djs = JSON.parse(localStorage.sy_djs);
        var sy_djs_xg_dx = sy_djs['sy_djs' + sy_nb_zhixian];
        input_djs_tjym_sj.value = sy_djs_xg_dx[0];
        var time_day = new Date(sy_djs_xg_dx[1]);
        input_djs_tjym_time_s = document.querySelectorAll('.input_djs_tjym_time_s');
        input_djs_tjym_time_s[0].value = time_day.getFullYear();
        input_djs_tjym_time_s[1].value = (time_day.getMonth() + 1 < 10 ? '0' + (time_day.getMonth() + 1) : time_day.getMonth() + 1);
        input_djs_tjym_time_s[2].value = (time_day.getDate() < 10 ? '0' + time_day.getDate() : time_day.getDate());
...
function ke_biao_shuchu() { //输出课表
    var ke_biao = JSON.parse(localStorage.ke_biao);
    var sy_ke_biao_max = document.querySelector('.sy_ke_biao_max');

    const syKeBiaoHList = sy_ke_biao_max.querySelectorAll('.sy_ke_biao_h');
    syKeBiaoHList.forEach(element => element.remove());
...
function zhou_zhi_zd() {
    if (localStorage.ke_biao_sd == 'true') {
        var sy_ke_biao_l = document.querySelectorAll('.sy_ke_biao_l');
        var ke_biao_zhoushu = document.querySelector('.ke_biao_zhoushu').value;
        var dayOfWeek = new Date().getDay();

        for (let i = 0; i < sy_ke_biao_l.length; i++) {
            var ke_biao_zhoushu2 = ke_biao_zhoushu;
            if ((((i + 1) % 8) == 0 ? 7 : ((i + 1) % 8) - 1) < (dayOfWeek == 0 ? 7 : dayOfWeek)) { //属于下一周
                ke_biao_zhoushu2++;
            }

            // 周数范围
            function extractWeekRange(str) { // 正则匹配方括号内的周数范围（如[1-13周]）
                const weekRangeMatch = str.match(/\[(\d+-\d+)周\]/);
                if (!weekRangeMatch) return [];
                return weekRangeMatch[1].split('-').map(Number);
            }
            var zhou = extractWeekRange(sy_ke_biao_l[i].value);
            if (zhou.length > 0 && localStorage.ke_biao_zfw == 'true') {
            ...
            // 单双周
            if ((sy_ke_biao_l[i].value).indexOf('[单周]') !== -1 && ke_biao_zhoushu2 % 2 == 0 && localStorage.ke_biao_dsz == 'true') {
                sy_ke_biao_l[i].value = '';
            }
...
function AI_zdcc_ks(ksczzz) {
    try {
        ...
        } else if (ksczzz.substring(0, 6) == '打开URL;' && ksczzz.split(';', 2)[1].startsWith('http')) {
            document.querySelector('.so_ssk').value = (ksczzz.split(';', 2)[1]);
            document.querySelector('.so_anniu').click();
        } else {
            Sku_tctx('抱歉,AI又在胡编乱造指令,优化中');
